...
public class Interpreter {
    ...
    private int bloco = 0;
    public static VarList lista = new VarList();
    ...
    public void error(String msg) {
        System.out.println("Erro!\n" + msg + "\nUltimo Token lido: " + look + " na linha: " + look.linha + " e coluna: "
                + look.coluna);
        System.exit(0);
    }
    ...
    private void bloco() {
        if (match(TokenTypes.ABRE_CHAVE)) {
            ...
            while (match(TokenTypes.IDENTIFICADOR) || match(TokenTypes.WHILE_ID) || match(TokenTypes.DO_ID)
                    || match(TokenTypes.IF_ID) || match(TokenTypes.FOR_ID) || match(TokenTypes.PRINT)
                    || match(TokenTypes.STRLEN)) {
                comando();
            }
    ...
    public TokenTypes op_relacional() {
        if (!(match(TokenTypes.MENOR_IGUAL) || match(TokenTypes.MENOR) || match(TokenTypes.MAIOR_IGUAL)
                || match(TokenTypes.MAIOR) || match(TokenTypes.IGUAL) || match(TokenTypes.DIFERENTE))) {
    ...
    public double fator() {
        ...
        if (match(TokenTypes.STRLEN)) {
            valor = strlen(false);
        } else if (match(TokenTypes.IDENTIFICADOR) || match(TokenTypes.INT) || match(TokenTypes.FLOAT)
                || match(TokenTypes.CHAR)) {
            if (match(TokenTypes.IDENTIFICADOR)) {
        ...
        } else if (match(TokenTypes.MULTIPLICACAO) || match(TokenTypes.DIVISAO) || match(TokenTypes.SOMA)
                || match(TokenTypes.SUBTRACAO) || match(TokenTypes.PONTO_VIRGULA)) {
        ...
        if (match(TokenTypes.ABRE_PARENTESES)) {
            ...
            if (match(TokenTypes.ASPAS)) {
                ...
                if (match(TokenTypes.ASPAS)) {
                    ...
                    error("não fechou aspas do Sout");
                }
            ...
            if (match(TokenTypes.FECHA_PARENTESES)) {
                ...
                error("não fechou parenteses do Sout");
            }
        } else {
            error("não abriu parenteses no Sout");
        }
    }
